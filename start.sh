...
get_time=`curl --silent http://$ev_ip/cm?cmnd=time%202`
get_time_epoch=`printf "$get_time" | jq .Epoch`

# Disable all other timers first
jq_payload="{\"Enable\":0}"
jq_payload_encoded=`printf "%s" $jq_payload | jq -sRr @uri`
curl --silent http://$ev_ip/cm?cmnd=Timer1%20$jq_payload_encoded
curl --silent http://$ev_ip/cm?cmnd=Timer2%20$jq_payload_encoded
curl --silent http://$ev_ip/cm?cmnd=Timer3%20$jq_payload_encoded
...
# curl --silent $ev_ip/cm?cmnd=Timer1%20$opop
...
curl --silent http://$ev_ip/cm?cmnd=Power%201 http://$ev_ip/cm?cmnd=Timer4%20$jq_payload_end_encoded
